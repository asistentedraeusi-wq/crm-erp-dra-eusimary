...
// Envía el Kit del Paciente como PDF adjunto al email del paciente vía Brevo
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const { email, nombre, pdfBase64 } = await req.json() as {
      email:      string
      nombre:     string
      pdfBase64?: string
    }
...
<body style="margin:0;padding:0;background:#F4F7FA;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#F4F7FA;padding:40px 20px;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 4px 24px rgba(0,0,0,0.08);">
        ...
        <tr><td style="background:#12C49A;padding:12px 40px;text-align:center;">
          <p style="margin:0;color:#fff;font-size:13px;font-weight:700;">🩺 TU KIT DEL PACIENTE — PLAN PERSONALIZADO</p>
        </td></tr>
        <tr><td style="padding:36px 40px;">
          <h2 style="margin:0 0 16px;color:#0D2244;font-size:20px;font-weight:700;">Hola, ${primerNombre} 👋</h2>
          <p style="margin:0 0 20px;color:#4B5563;font-size:15px;line-height:1.7;">
            La Dra. Eusimary Contreras ha preparado tu <strong>Kit del Paciente</strong> con tu plan de manejo
            personalizado: alimentación, ejercicio, suplementación y medicación.
          </p>
          <div style="background:#F0FDF4;border:1px solid #BBF7D0;border-radius:10px;padding:16px 20px;margin-bottom:20px;">
            <p style="margin:0;color:#065F46;font-size:13px;font-weight:700;">📎 Tu Kit del Paciente está adjunto a este correo en formato PDF.</p>
          </div>
          <p style="margin:0;color:#6B7280;font-size:13px;line-height:1.7;">
            Guarda este documento — es tu guía personal para todo el programa. Si tienes preguntas,
    ...
    const payload: Record<string, unknown> = {
      ...
      subject:     `🩺 Tu Kit del Paciente — Plan Personalizado · Dra. Eusimary Contreras`,
    ...
    if (pdfBase64) {
      payload.attachment = [{
        content: pdfBase64,
        name:    `Kit_Paciente_${(nombre || 'Paciente').replace(/\s+/g, '_')}.pdf`,
      }]
    }
